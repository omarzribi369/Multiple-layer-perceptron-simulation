...
def forward(x1, x2, ww):
    zh1 = x1*ww['w00'] + x2*ww['w01'] -0.1 ;  h1 = sigmoid(zh1)
    zh2 = x1*ww['w10'] + x2*ww['w11'] -0.1;  h2 = sigmoid(zh2)
    zo  = h1*ww['v0']  + h2*ww['v1'] -0.1;   yp = sigmoid(zo)
...
def compute_grads(x1, x2, y, r, ww):
    ...
    return {
        'dL_dy': dL_dy, 'dy_dzo': dy_dzo, 'delta_o': delta_o,
        'ddv0': ddv0, 'ddv1': ddv1, 'dh1': dh1, 'dh2': dh2,
        'dw00': dh1*x1, 'dw01': dh1*x2,
        'dw10': dh2*x1, 'dw11': dh2*x2,
    }

def apply_step(ww, bp, lr):
    return {
        'w00': ww['w00'] - lr*bp['dw00'], 'w01': ww['w01'] - lr*bp['dw01'],
        'w10': ww['w10'] - lr*bp['dw10'], 'w11': ww['w11'] - lr*bp['dw11'],
        'v0':  ww['v0']  - lr*bp['ddv0'], 'v1':  ww['v1']  - lr*bp['ddv1'],
    }

#  session state 
if 'w'            not in st.session_state: st.session_state.w            = rand_weights()
if 'iteration'    not in st.session_state: st.session_state.iteration    = 0
if 'x1'           not in st.session_state: st.session_state.x1           = 0.5
if 'x2'           not in st.session_state: st.session_state.x2           = 0.8
if 'y_true'       not in st.session_state: st.session_state.y_true       = 0.7
if 'loss_history' not in st.session_state: st.session_state.loss_history = []
if 'w_history'    not in st.session_state: st.session_state.w_history    = []
...
with ctrl_col:
    ...
    st.markdown(f"**Loss:** `{loss:.6f}`")

with net_col:
    st.markdown('<div class="sec">Network</div>', unsafe_allow_html=True)

    fig, ax = plt.subplots(figsize=(11, 3.8))
    ax.set_xlim(0,11); ax.set_ylim(0,4.8); ax.set_aspect('equal'); ax.axis('off')
...
with col_tbl:
    ...
    st.dataframe(pd.DataFrame({
        'Variable': ['x₁','x₂','y (true)','ŷ (pred)','Loss'],
...
with col_eq:
    ...
    eq_blocks = [
        ...
        (None, r"$z_o = h_1 v_0 + h_2 v_1$", r"$\hat{y} = \sigma(z_o)$"),
        (None, r"$\text{Loss} = (y - \hat{y})$", None),
        ("Chain rule", None, None),
        (None, r"$\frac{\partial \text{Loss}}{\partial v} = \frac{\partial \text{Loss}}{\partial \hat{y}} \cdot \frac{\partial \hat{y}}{\partial z_o} \cdot \frac{\partial z_o}{\partial v}$", None),
        (None, r"$\frac{\partial \text{Loss}}{\partial w} = \frac{\partial \text{Loss}}{\partial \hat{y}} \cdot \frac{\partial \hat{y}}{\partial z_o} \cdot \frac{\partial z_o}{\partial h} \cdot \frac{\partial h}{\partial z_h} \cdot \frac{\partial z_h}{\partial w}$", None),
        ("Backprop — output layer", None, None),
        (None, r"$\frac{\partial \text{Loss}}{\partial \hat{y}} = -1$", r"$\sigma'(z_o) = \hat{y}(1-\hat{y})$"),
        (None, r"$\delta_o = \frac{\partial \text{Loss}}{\partial \hat{y}} \cdot \sigma'(z_o)$", None),
        (None, r"$\frac{\partial \text{Loss}}{\partial v_0} = \delta_o \cdot h_1$", r"$\frac{\partial \text{Loss}}{\partial v_1} = \delta_o \cdot h_2$"),
        ...
        (None, r"$\frac{\partial \text{Loss}}{\partial w_{00}} = \delta_{h_1} \cdot x_1$", r"$\frac{\partial \text{Loss}}{\partial w_{01}} = \delta_{h_1} \cdot x_2$"),
        (None, r"$\frac{\partial \text{Loss}}{\partial w_{10}} = \delta_{h_2} \cdot x_1$", r"$\frac{\partial \text{Loss}}{\partial w_{11}} = \delta_{h_2} \cdot x_2$"),
        ("Gradient descent", None, None),
        (None, r"$v \leftarrow v - \eta \cdot \frac{\partial \text{Loss}}{\partial v}$",r"$w \leftarrow w - \eta \cdot \frac{\partial \text{Loss}}{\partial w}$"),
    ]
    ...
    fig2.patch.set_facecolor('#fafaf7'); ax2.set_facecolor('#fafaf7')
    ax2.axis('off'); ax2.set_xlim(0, 10); ax2.set_ylim(0, total_u)
    yc = total_u - 0.15
    for (sec, leq, req) in eq_blocks:
        if sec is not None:
            ...
            if leq: ax2.text(0.1, yc, leq, ha='left', va='center', fontsize=11, color='#1a1a2e')
            if req: ax2.text(5.1, yc, req, ha='left', va='center', fontsize=11, color='#1a1a2e')
            yc -= 0.45
...
with col_it:
    ...
    html += f"zₕ₁={x1}·{w['w00']:.3f}+{x2}·{w['w01']:.3f}-0.1 =<b>{zh1:.4f}</b>{p}h₁=σ(zₕ₁)=<b>{h1:.4f}</b><br>"
    html += f"zₕ₂={x1}·{w['w10']:.3f}+{x2}·{w['w11']:.3f}-0.1 =<b>{zh2:.4f}</b>{p}h₂=σ(zₕ₂)=<b>{h2:.4f}</b><br>"
    html += f"z_o={h1:.4f}·{w['v0']:.3f}+{h2:.4f}·{w['v1']:.3f}-0.1 =<b>{zo:.4f}</b>{p}ŷ=σ(z_o)=<b>{yp:.4f}</b><br>"

    html += f"<br><b>Loss</b> &nbsp; Loss = ({y}−{yp:.4f})² = <b>{loss:.6f}</b><br>"

    html += "<br><b>Backprop — output layer</b><br>"
    html += f"∂Loss/∂ŷ = −2({y}−{yp:.4f}) = <b>{bp['dL_dy']:.4f}</b><br>"
    html += f"σ'(z_o) = ŷ(1−ŷ) = {yp:.4f}·(1−{yp:.4f}) = <b>{bp['dy_dzo']:.4f}</b><br>"
    html += f"δ_o = ∂Loss/∂ŷ · σ'(z_o) = {bp['dL_dy']:.4f}·{bp['dy_dzo']:.4f} = <b>{bp['delta_o']:.4f}</b><br>"
    html += f"∂Loss/∂v₀=δ_o·h₁=<b>{bp['ddv0']:.4f}</b>{p}∂Loss/∂v₁=δ_o·h₂=<b>{bp['ddv1']:.4f}</b><br>"

    html += "<br><b>Backprop — hidden layer</b><br>"
    html += f"δ_h₁=δ_o·v₀·σ'(zₕ₁)={bp['delta_o']:.4f}·{w['v0']:.4f}·{h1:.4f}·(1−{h1:.4f})=<b>{bp['dh1']:.4f}</b><br>"
    html += f"δ_h₂=δ_o·v₁·σ'(zₕ₂)={bp['delta_o']:.4f}·{w['v1']:.4f}·{h2:.4f}·(1−{h2:.4f})=<b>{bp['dh2']:.4f}</b><br>"
    html += f"∂Loss/∂w₀₀=δ_h₁·x₁=<b>{bp['dw00']:.4f}</b>{p}∂Loss/∂w₀₁=δ_h₁·x₂=<b>{bp['dw01']:.4f}</b><br>"
    html += f"∂Loss/∂w₁₀=δ_h₂·x₁=<b>{bp['dw10']:.4f}</b>{p}∂Loss/∂w₁₁=δ_h₂·x₂=<b>{bp['dw11']:.4f}</b><br>"

    html += f"<br><b>Gradient descent (η={lr})</b><br>"
    html += f"v₀:{w['v0']:.4f}−{lr}·{bp['ddv0']:.4f}=<b>{w_new['v0']:.4f}</b>{p}v₁:{w['v1']:.4f}−{lr}·{bp['ddv1']:.4f}=<b>{w_new['v1']:.4f}</b><br>"
    html += f"w₀₀:{w['w00']:.4f}−{lr}·{bp['dw00']:.4f}=<b>{w_new['w00']:.4f}</b>{p}w₀₁:{w['w01']:.4f}−{lr}·{bp['dw01']:.4f}=<b>{w_new['w01']:.4f}</b><br>"
    html += f"w₁₀:{w['w10']:.4f}−{lr}·{bp['dw10']:.4f}=<b>{w_new['w10']:.4f}</b>{p}w₁₁:{w['w11']:.4f}−{lr}·{bp['dw11']:.4f}=<b>{w_new['w11']:.4f}</b><br>"

    html += f"<br><b>Loss: {loss:.6f}</b></div>"
